...
@Component({
  selector: 'hi-select-button',
  templateUrl: './select-button.template.html',
  changeDetection: ChangeDetectionStrategy.OnPush,
  host: {
    '[class]': '(cssClass || "") + " btn-group " + (size ? "btn-group-" + size : "") ',
    'role': 'group',
...
export class SelectButtonComponent implements ControlValueAccessor {
  ...
  constructor(private changeDetectorRef: ChangeDetectorRef, hiNGConfig: HiNGConfig) {
    this.type = <any>hiNGConfig.selectButton.type;
    this.justified = hiNGConfig.selectButton.justified;
    this.multiple = hiNGConfig.selectButton.multiple;
  }
