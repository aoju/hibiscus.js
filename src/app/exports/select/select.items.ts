...
import {NgOption} from './select.service';
import {SelectModel} from './select.model';
import {isDefined, isFunction, isObject, newId, stripChars} from './select.utils';
...
export class SelectItems {
  ...
  constructor(
    private _ngSelect: SelectComponent,
    private _selectionModel: SelectModel) {
  }
  ...
  findByLabel(term: string) {
    term = stripChars(term).toLocaleLowerCase();
    return this.filteredItems.find(item => {
      const label = stripChars(item.label).toLocaleLowerCase();
  ...
  filter(term: string): void {
    ...
    this._filteredItems = [];
    term = this._ngSelect.searchFn ? term : stripChars(term).toLocaleLowerCase();
  ...
  private _defaultSearchFn(search: string, opt: NgOption) {
    const label = stripChars(opt.label).toLocaleLowerCase();
